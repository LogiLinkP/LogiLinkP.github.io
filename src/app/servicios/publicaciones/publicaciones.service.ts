...
export class PublicacionesService {
  ...
  nueva_publicacion(id_encargado:number, id_carrera:number, titulo:string, enunciado:string, fecha:any, isfijo:boolean, fecha_programada:any){
    const req = new HttpRequest('POST', `${environment.url_back}/publicacion/crear`, {id_encargado, id_carrera, titulo, enunciado, fecha, isfijo, fecha_programada}, {responseType:"json"});
    return this._http.request(req);
  }

  editar_publciacion(id:number, titulo:string, enunciado:string, isfijo:boolean){
    const req = new HttpRequest('PUT', `${environment.url_back}/publicacion/editar`, {id, titulo, enunciado, isfijo}, {responseType:"json"});
    return this._http.request(req);
  }

  eliminar_publicacion(id:number){
    const req = new HttpRequest('DELETE', `${environment.url_back}/publicacion/eliminar?id=${id}`, {id}, {responseType:"json"});
    return this._http.request(req);
  }
}
