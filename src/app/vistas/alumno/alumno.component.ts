...
export class DetalleAlumnoComponent implements OnInit{
  ...
  finalizar_practica(practica: any) {
    ...
    this.service_gestion.finalizar_practica(practica.id_estudiante, practica.id, environment.estado_practica.finalizada, correo_supervisor, nom_estudiante).subscribe({
      ...
      complete: () => {
        let respuesta: any = [];
        this.service_noti.postnotificacion(id_encargado, "El alumno " + this.estudiante.nombre + " ha finalizado su práctica y desea su revisión", correo_encargado, this.estado_config).subscribe({
          next:(data:any) => {
            respuesta = {...respuesta, ...data};
          },
          error:(error:any) => {
